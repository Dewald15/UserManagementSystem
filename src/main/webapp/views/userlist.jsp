    ...
    <table class="table table-bordered table-hover table-responsive-sm">
        ...
        <c:forEach var="user" items="${userList}">
            <tr>
                ...
                <td class="align-middle">${user.domain}</td>
                <td class="actions-column">
                    <a href="<%= request.getContextPath() %>/RemoveUserController?email=${user.email}">Remove</a>
                    <a href="<%= request.getContextPath() %>/views/edit.jsp?name=${user.name}&email=${user.email}&mobilenumber=${user.contact}&domain=${user.domain}">Edit</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
